...
export const FLUID_DENSITY_RANGES = [
...
export interface FluidRangeWithColor {
  name: string;
  min: number;
  max: number;
  color: string;
...
export interface FluidDatabase {
  [key: string]: FluidRange;
}

// Convert FLUID_DENSITY_RANGES to FluidDatabase format
export const getDefaultFluidDatabase = (): FluidDatabase => {
  const database: FluidDatabase = {};
...
export const getFluidColor = (fluidName: string): string => {
...
export const detectFluidFromDatabase = (density: number, fluidDatabase: FluidDatabase): { name: string; color: string } => {
  ...
  let closestFluid: { name: string; color: string } | null = null;
