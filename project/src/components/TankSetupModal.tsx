...
interface TankSetupModalProps {
  email: string;
  onComplete: () => void;
}

export function TankSetupModal({ email, onComplete }: TankSetupModalProps) {
  const [numTanks, setNumTanks] = useState(1);
  const [vesselName, setVesselName] = useState('');
  const [tankConfigs, setTankConfigs] = useState<Partial<TankSettings>[]>([
    { 
      vesselName: '', 
      tankName: 'Tank1', 
      shape: 'Cylindrical',
  ...
  const { saveTankSettings, loading, error: dataError } = useData();

  const updateTankConfig = (index: number, field: keyof TankSettings, value: any) => {
  ...
  const handleNumTanksChange = (num: number) => {
    setNumTanks(num);
  ...
  const handleVesselNameChange = (name: string) => {
    setVesselName(name);
  ...
  const handleSave = async () => {
    ...
    if (validConfigs.length !== numTanks) {
      setError('Please fill in all required fields for all tanks');
      return;
    }
    ...
    setError('');
    try {
      await saveTankSettings(email, validConfigs);
      onComplete();
    } catch (err) {
      setError('Failed to save tank settings. Please try again.');
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          <div className="mt-6 flex justify-end space-x-3">
            ...
            >
              {(isSaving || loading) ? 'Saving...' : 'Save Tank Settings'}
            </button>
          </div>
        </div>
      </div>
